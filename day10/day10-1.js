// input: {x,y,direction} and tile 
const makeMove = (status, tile) => {
    // create new status based on current location and direction and tile
    switch (tile) {
        case 'J':
            if (status.direction == 'down') {
                status.direction = 'left'
                status.x -= 1
            } else if (status.direction == 'right') {
                status.direction = 'up'
                status.y -= 1
            } else {
                console.log(`tile ${tile} and direction ${status.direction} are not compatible`)
            } break;
        case '7':
        ...
        case '|':
            ...
            break;
        default:
            console.log(`Sorry, ${tile} not found. Not Good `);
...
const produceResult = () => {
    ...
    const moveThroughLoop = (status, stepsMade, maxMovesStillToGo, destination) => {
        let newStatus = status
        while (maxMovesStillToGo-- > 0) {
            // console.log(`make move ${JSON.stringify(status)} using tile ${ lines[status.y][status.x]}`)
            newStatus = makeMove(newStatus, lines[newStatus.y][newStatus.x])
            stepsMade++
            if (newStatus.x == destination.x && newStatus.y == destination.y) {
                console.log(`Reached S after ${stepsMade} moves`)
                return
            }
        }
        console.log(`No more moves available to make`)
        return
    }

    const maxMovesToMake = 50000
    const lines = inputDoc.split('\n');
    // find S
    const locationOfS = inputDoc.replace(/\n/g, "").indexOf('S')
    let location = {
        y: Math.trunc((locationOfS + 1) / lines[0].length)
        , x: (locationOfS) % lines[0].length

    }
    console.log(`S = [${location.x},${location.y}]`)
    // inspect four tiles around S; try to find loop for each - provided:
    // tile != .
    // tile above or under S must not be - and tile left or right of S cannot be | 
    let newStatus
    if (location.x > 0 && !"|.7J".includes(lines[location.y][location.x - 1])) {
        console.log(`let's try left`)
        moveThroughLoop({ x: location.x - 1, y: location.y, direction: 'left' }, 1, maxMovesToMake, location)
    }
    if (location.x < lines[0].length - 1 && !"|.FL".includes(lines[location.y][location.x + 1])) {
        console.log(`let's try right`)
        moveThroughLoop({ x: location.x + 1, y: location.y, direction: 'right' }, 1, maxMovesToMake, location)
    }
    if (location.y < lines.length - 1 && !"-.F7".includes(lines[location.y + 1][location.x])) {
        console.log(`let's try down`)

        moveThroughLoop({ x: location.x, y: location.y + 1, direction: 'down' }, 1, maxMovesToMake, location)
    }
    if (location.y > 0 && !"LJ-.".includes(lines[location.y - 1][location.x])) {
        console.log(`let's try up`)
        moveThroughLoop({ x: location.x, y: location.y - 1, direction: 'up' }, 1, maxMovesToMake, location)
    }
}

const inputDoc2 = `7-F7-
.FJ|7
SJLL7
|F--J
LJ.LJ`

const inputDoc = `F-F-JJF7.F7.F-LJ7-|77.L|.FJ7--7LFL|-|F-.|--7.F7-F-.F|-FL7.FF7.F77-FLLJFLF--.FL--FF--7..FJ-L-LL-J..F-J77FF|-FF77FL|-|7J-7--7J-L77.FL.FL|-F7F-
