...
// create a matrix of booleans (hotzones); these booleans indicate whether a specific position is active: if a number that touches that position is one that counts towards the total
//   a hotzone is defined by a symbol (any character that is not a digit 0-9 or a period). 
//   for a symbol at position (x,y), the hotzone is defined by the square that is bounded by top-left (x-1,y-1) and bottom-right (x+1, y+1)  
// loop over all lines; 
//   find (starting positions of) number strings;
//   for each string, check if at least one of its positions matches a hotposition; if it does, then this number can be added to the total
// NOTE: X is horizontal, within lines; Y is vertical - cross lines!


const produceResult = () => {

    const createHotzone = (hotX, hotY, maxX, maxY) => {
        for (let x = Math.max(0, hotX - 1); x <= Math.min(maxX, hotX + 1); x++) {
            for (let y = Math.max(0, hotY - 1); y <= Math.min(maxY, hotY + 1); y++) {
                hotPositions[x][y] = true
            }
        }
    }
    ...
    //hotPositions is a matrix of booleans that indicate whether a specific position is active
    const hotPositions = Array.from({ length: maxLineLength }, () => Array(lines.length).fill(false));

    let lineIndex = 0;
    for (const line of lines) {
        // find symbols in the line and create hotzone around symbol
        for (let i = 0; i < line.length; i++) {
            if (!line[i].match(/[0-9.]/)) {
                createHotzone(i,lineIndex,  maxLineLength, lines.length)
            }
        }
        lineIndex++
    }
    console.log(`Hotpositions: \n${JSON.stringify(hotPositions).replace(/false/g, '-').replace(/true/g, '+').replace(/,/g, '').replace(/]/g, '\n ')}`)
    lineIndex = 0;
    ...
    for (const line of lines) {
        ...
        while ((match = digitRegex.exec(line)) !== null) {
            for (let pos=match.index; pos < match.index + match[0].length; pos++) {
                if (hotPositions[pos][lineIndex]){
                    // this number can be added
                    partnumberTotal+= parseInt(match[0])
                    break;
                }
            } 
        }
...
inputDoc=`.......497...........................858...923...128..................227..801........487.....664...........................................
