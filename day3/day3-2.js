...
//   determine number of lines (height of matrix) and longest line length (width of matrix)
// create a matrix of sets (hotzones); these sets indicate for which stars/gears a specific position is active: if a number that touches that position is one that is part of a gear ratio
//   note: a position can participate in the hotzone for more than one gear; for each hot position, we need to record each of the gears/stars it is associated with
//   a hotzone is defined by a * symbol . 
//   for a * at position (x,y), the hotzone is defined by the square that is bounded by top-left (x-1,y-1) and bottom-right (x+1, y+1)  
// loop over all lines; 
//   find (starting positions of) number strings;
//   for each string, check if at least one of its positions matches a hotposition; if it does, then this number potentially participates in the gear ratio for that * symbol
//      for each * the position is active for, record the numbers that participate in its gear ratio.
//   loop over all gears; see which ones have two numbers associated with them 
//      for these gears, calculate the product of the two numbers and add to the total
// NOTE: X is horizontal, within lines; Y is vertical - cross lines!


const produceResult = () => {

    const createHotzone = (hotX, hotY, maxX, maxY, starId) => {
        for (let y = Math.max(0, hotY - 1); y <= Math.min(maxY, hotY+1); y++) {
            for (let x = Math.max(0, hotX - 1); x <= Math.min(maxX, hotX+1); x++) {
                if (hotPositions[y][x]) {
                    hotPositions[y][x].push(starId)
                } else hotPositions[y][x]=[starId]
                console.log(`added ${starId} to set at ${y},${x} ; ${hotPositions[y][x].length > 1 ? " This position contains " + hotPositions[y][x].length + " elements " : ""} `)
            }
        }
    }

    const lines = inputDoc.split('\n');
    const maxLineLength = lines.reduce((max, str) => Math.max(max, str.length), 0);
    //hotPositions is a matrix of arrays that hold the indexes of the stars for which the position is active
    const hotPositions = Array.from({ length: lines.length }, () => Array(maxLineLength));
    let lineIndex = 0;
    let starCount = 0;
    for (const line of lines) {
        // find symbols in the line and create hotzone around gear symbol
        for (let pos = 0; pos < line.length; pos++) {
            if (line[pos] == '*') {
                console.log(`found a gear in line ${lineIndex} at position ${pos}`)
                createHotzone(pos, lineIndex, maxLineLength, lines.length, starCount++)

            }
        }
        lineIndex++
    }
    console.log(`Number of gears: ${starCount}`)

    const gears = Array(starCount)
    lineIndex = 0;
    for (const line of lines) {
        // find number strings in the lines
        let digitRegex = /\d+/g; // regular expression to find string of consecutive digits
        let match;
        while ((match = digitRegex.exec(line)) !== null) {
            for (let pos = match.index; pos < match.index + match[0].length; pos++) {
                if (hotPositions[lineIndex][pos] ) {
                    // this number can be added to the gears indicated by the set in hotPosition
                    for (const gear of hotPositions[lineIndex][pos]) {
                        console.log(`add ${match[0]} to gear ${gear} because of ${lineIndex}, ${pos}`);
                        if (gears[gear]){
                            gears[gear].push(match[0]);
                        } else gears[gear]= [match[0]]
                    }
                    break;
                }
            }
    ...
    for (const gear of gears) {
        if (gear.length>1) {
            console.log(`gear ${gearIndex} has ${gear.length} elements`)
            gearRatioTotal+= gear[0]*gear[1]
        }
        gearIndex++
    }
    console.log(`Gears ${JSON.stringify(gears)}`)
    console.log(`gear ratio total ${gearRatioTotal} `)

}

const inputDoc2 = `467..114..
...*......
..35..633.
......#...
617*......
...
const inputDoc = `.......497...........................858...923...128..................227..801........487.....664...........................................
