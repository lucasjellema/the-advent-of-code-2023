...
//      for each game:
//        reset minColorCounts (all of them zero) 
//        split on , for the color+count draw
//        for each draw
//          split on ' ' resulting in the count and the color count
//          check if count > min count for that color; if so, update the min count to the count
//        calculate power of cubes: multiply the min counts for the colors
//        add cube power to powertotal
...
const produceResult = () => {
    ...
    for (const line of lines) {
        ...
        const minColorCounts = { "red": 0, "green": 0, "blue": 0 }
        ...
        const cubePower =  minColorCounts["red"] * minColorCounts["blue"] * minColorCounts["green"]
        powerTotal+= cubePower
    }
