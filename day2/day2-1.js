...
// define max number of each color
// set gametotal to 0
// loop over all array elements -lines:
// for each line, 
//    split on :
//      retrieve game id from first section (remove "Game ")
//      split second section on ; - resulting in games
//      for each game:
//        split on , for the color+count draw
//        for each draw
//          split on ' ' resulting in the count and the color count
//          check if count > max count for that color; if so, this game does not qualify 
//        if the game was not disqualified then add game id to gametotal

const maxColorCounts = { "red": 12, "green": 13, "blue": 14, }
...
const produceResult = () => {
    ...
    for (const line of lines) {
        ...
        const gameId = lineSegments[0].replace(/Game /, '').split(':');
        const draws = lineSegments[1].split(';');
        let gameDisqualified = false;
        for (const draw of draws) {
            const pairs = draw.split(',');
            for (const pair of pairs) {
                const colorAndCount = pair.split(' ');
                const color = colorAndCount[2]
                const count = parseInt(colorAndCount[1])
                if (count > maxColorCounts[color]) {
                    console.log(`game disqualified ${gameId} color ${color}: ${count} > ${maxColorCounts[color]}`)
                    gameDisqualified = true
                }
            }
        }
        if (!gameDisqualified) { gameTotal+= parseInt(gameId)}
    }
    console.log(`Game Total ${gameTotal}`)
}
const inputDoc2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`

const inputDoc = `Game 1: 1 blue, 1 red; 10 red; 8 red, 1 blue, 1 green; 1 green, 5 blue
