...
-- Note: first run the DDL and DML statements below to create tables and load data
...
with line_sums as
( select to_number( REGEXP_SUBSTR(line, '\d') 
                    ||
                    REGEXP_SUBSTR(REVERSE(line), '\d')
                  ) AS line_sum
  from   lines
)
select sum(line_sum)
...
create table lines (line varchar2(2000));

create table doc (content clob);

insert into doc (content) values ('2911threeninesdvxvheightwobm
