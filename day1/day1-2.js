...
// for each element (or line),
//    find replace all written numbers by their numerical equivalent 
//        find occurrences of spelled out numbers; replace the first one found; then recursively process again until no more occurrences are found (three => 3, eight => 8)
//    determine first number and last number
//    create a string by concatenating first and last number 
//    add the numeric value of that string to the total

const replacements = [
  { regexpPattern: /one/, replacingNumber: '1ne' }
, { regexpPattern: /two/, replacingNumber: '2wo' }
, { regexpPattern: /three/, replacingNumber: '3hree' }
, { regexpPattern: /four/, replacingNumber: '4our' }
, { regexpPattern: /five/, replacingNumber: '5ive' }
, { regexpPattern: /six/, replacingNumber: '6ix' }
, { regexpPattern: /seven/, replacingNumber: '7even' }
, { regexpPattern: /eight/, replacingNumber: '8ight' }
, { regexpPattern: /nine/, replacingNumber: '9ine'  }
]

const convertWrittenNumbersToDigits = (doc) => {
    // replace first occurrences of a spelled out number its digital equivalent
    let firstReplacementIndex
    let firstReplacement
    let found = false;
    for (const replacement of replacements) {
        let index = doc.search(replacement.regexpPattern)
        if (index > -1 && (index < firstReplacementIndex || !found)) {
            found = true
            firstReplacementIndex = index
            firstReplacement = replacement
        }
    }
    if (found) {
        return convertWrittenNumbersToDigits(doc.replace(firstReplacement.regexpPattern, firstReplacement.replacingNumber))
    } else
        return doc
}
...
const produceResult = () => {
    ...
    for (const line of lines) {
        const lineWithNumbers = convertWrittenNumbersToDigits(line)
        const numbersInLine = lineWithNumbers.match(/\d/g)
        console.log(`line = ${line}; first number: ${numbersInLine[0]}, last number : ${numbersInLine[numbersInLine.length - 1]}`)
        total += parseInt(`${numbersInLine[0]}${numbersInLine[numbersInLine.length - 1]}`)
    }
    console.log(`total: ${total}`)
}


const inputDoc2 = `two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
...
const inputDoc = `2911threeninesdvxvheightwobm
3three16xsxhpnqmzmnine8one
seven5khtwo891hlb
sixthreeqpzjpn195
jrnf3
